<template>
  <div class="f-modal" transition="v-slide-y-reverse-transition">
    ...
    <div class="profile-wrapper" v-if="profile">
      ...
      <h6>{{ profile.name }}, {{ age }}</h6>
      <div class="background">
        <p class="job bg-item" v-for="(job, index) in profile.jobs">
          <i :class="{'icon-briefcase': index === 0}"></i>
          <span v-if="job.title">{{ job.title.name }}</span>
          <span v-if="job.title && job.company">at</span> 
          <span v-if="job.company">{{ job.company.name }}</span>
        </p>
        <p class="school bg-item" v-for="(school, index) in profile.schools">
          <i :class="{'icon-book': index === 0}"></i>
          {{ school.name }}
        </p>
        <p class="bg-item"><i class="icon-location-2"></i> {{ distance }} kilometers away</p>
      </div>
